...
import { updatebillingaddress } from '../services/services';
...
const Billingbox = React.forwardRef((props, ref) => {
  useLayoutEffect(() => {
    const user = JSON.parse(secureLocalStorage.getItem('user_info'));

    props.setformobj({
      firstname: user.billing_fname,
      lastname: user.billing_lname,
      streetaddress: user.billing_address1,
      streetaddress2: user.billing_address2,
      zipcode: user.billing_zip,
      city: user.billing_city,
      state: user.billing_state_id,
    });
  }, []);
  ...
  const updatebilling = () => {
    ...
    if (validate()) {
      updatebillingaddress(props.formobj).then((result) => {
        setloading(false);
        props.setrefresh((prev) => prev + 1);
        Swal.fire({
          icon: 'success',
          title: 'Success',
          text: 'Successfully updated billing address',
      ...
      <Paper className="relative flex flex-col flex-auto p-24 mb-24  rounded-2xl shadow overflow-hidden">
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <FormControlLabel
              control={<Checkbox onChange={() => props.setformobj(props.copy)} />}
              label="Use same as above"
            />
          </div>
          <div className="-mt-8" />
        </div>
...
export default memo(Billingbox);
