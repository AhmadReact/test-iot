...
import { createCustomerUpdated } from '../services/services';

import { useAddSimsAndPlansMutation } from '../services/Apis';
...
function SignUpPage() {
  ...
  const [addSimsAndPlans] = useAddSimsAndPlansMutation();
  ...
  // Extract plan_id and sim_id from URL params (can appear multiple times)
  const extractParams = useCallback(() => {
    const planIds = searchParams.getAll('plan_id').filter(Boolean);
    const simIds = searchParams.getAll('sim_id').filter(Boolean);
    return { planIds, simIds };
  }, [searchParams]);
  const navigate = useNavigate();
  const { planIds, simIds } = useMemo(() => extractParams(), [extractParams]);
    ...
    createCustomerUpdated(getValues(), searchParams.get('referral_code'))
      .then((result) => {
        ...
        if (result?.status === 'error' && result?.data) {
          ...
          // Collect all error messages for Swal
          const allErrorMessages = [];

          // Set field-specific errors
          Object.keys(result.data).forEach((fieldName) => {
            const errorMessages = result.data[fieldName];
            if (Array.isArray(errorMessages) && errorMessages.length > 0) {
              const errorMessage = errorMessages[0];
              // setError(fieldName, {
              //   type: "manual",
              //   message: errorMessage, // Use first error message
              // });
              // Collect error messages for Swal
          ...
          Swal.fire({
            ...
            html:
              allErrorMessages.length > 0
                ? `<ul style="text-align: left; padding-left: 20px;">${allErrorMessages
                    .map((msg) => `<li>${msg}</li>`)
                    .join('')}</ul>`
        ...
        // Only call addSimsAndPlans if we have plan_ids or sim_ids
        if ((planIds.length > 0 || simIds.length > 0) && result?.data?.id) {
          const formData = new FormData();
          formData.append('customer_id', result.data.id);

          // Add sim_ids with array indices
          simIds.forEach((simId, index) => {
            formData.append(`sim_ids[${index}]`, simId);
          });

          // Add plan_ids with array indices
          planIds.forEach((planId, index) => {
            formData.append(`plan_ids[${index}]`, planId);
          });

          // Call the mutation
          addSimsAndPlans(formData)
            .unwrap()
            .then(() => {
              reset();
            ...
            .catch((error) => {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Server Error',
                text: `User created successfully, but failed to add sims and plans: ${
                  error?.data?.message || error?.error || 'Unknown error'
                }`,
              });
            });
        } else {
          // No plan_ids or sim_ids, just show success
          reset();
          setSameShipBill(false);
          setLoader(false);
          Swal.fire({
            icon: 'success',
            title: 'Success',
            text: `User created and plans and sims added successfully. Please sign in to your account.`,
            confirmButtonText: 'Ok',
            preConfirm: () => {
              navigate('/sign-in');
            },
    ...
    <div className="flex flex-col sm:flex-row items-center md:items-start sm:justify-center md:justify-start flex-1 min-w-0">
      <Paper className="h-full sm:h-auto md:flex md:items-center md:justify-start w-full sm:w-auto md:h-full md:w-[100%] py-8 px-16 sm:p-48 md:p-64 sm:rounded-2xl md:rounded-none sm:shadow md:shadow-none ltr:border-r-1 rtl:border-l-1">
        <div className="w-full   mx-auto sm:mx-0">
          ...
          {(planIds.length > 0 || simIds.length > 0) && (
            <div className="mt-24 mb-12 p-16 border border-gray-200 rounded-lg bg-gray-50">
              <Typography className="font-semibold text-lg">Captured from your link</Typography>
              <Typography className="text-sm text-gray-600 mt-4">
                We detected these plan and SIM IDs in the URL.
              </Typography>
              <div className="flex flex-wrap gap-16 mt-12">
                <div className="flex items-center gap-8">
                  <Typography className="font-medium">Plan IDs:</Typography>
                  {planIds.length ? (
                    planIds.map((id) => (
                      <Chip
                        key={`plan-${id}`}
                ...
                <div className="flex items-center gap-8">
                  <Typography className="font-medium">SIM IDs:</Typography>
                  {simIds.length ? (
                    simIds.map((id) => (
                      <Chip
                        key={`sim-${id}`}
